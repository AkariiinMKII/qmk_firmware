...
// Static data
static const combo_config_t combos[] = { MNM_COMBO_DEFINITIONS };
static bool mod1_held = false;
static bool mod2_held = false;
#ifdef MNM_COMBO_ALLOW_OVER16
...
#else
_Static_assert(NUM_COMBOS <= 16, "Too many combo definitions for uint16_t bitmask (max 16, or define MNM_COMBO_ALLOW_OVER16 for up to 32)");
#endif
            ...
            if (pressed) {
                if (combo_ready()) {
                    ...
                    if (!was_satisfied) {
                        ...
                        unregister_code(MNM_COMBO_MOD1);
                        unregister_code(MNM_COMBO_MOD2);
...
bool mnm_combo_any_active(void) { return combo_satisfied_state != 0; }

...
static const rgblight_segment_t PROGMEM light_layer_blink[] = RGBLIGHT_LAYER_SEGMENTS({0, RGBLIGHT_LED_COUNT, AKC_COLOR_BLINK});
...
static const uint8_t layerled_positions[] = AKC_LAYERLED_POSITIONS;
static const uint8_t lockled_positions[] = AKC_LOCKLED_POSITIONS;
...
void akc_rgblight_layers_init(void) {
    ...
    for (uint8_t i = 0; i < 4; i++) {
        set_rgblight_layer_color(&light_layer_layerled_off[i], layerled_positions[i], AKC_LAYERLED_LENGTH, layer_bg_color);
    }
    ...
    for (uint8_t i = 0; i < 4; i++) {
        set_rgblight_layer_color(layer_on_segments[i], layerled_positions[i], AKC_LAYERLED_LENGTH, layer_on_color);
    }
    ...
    for (uint8_t i = 0; i < 3; i++) {
        ...
        set_rgblight_layer_color(lock_bg_segments[i], lockled_positions[i], AKC_LOCKLED_LENGTH, lock_bg_color);
    }
    ...
    for (uint8_t i = 0; i < 3; i++) {
        ...
        set_rgblight_layer_color(lock_on_segments[i], lockled_positions[i], AKC_LOCKLED_LENGTH, lock_on_color);
    }
